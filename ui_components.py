...
def render_kalustokortti(k, y_dict, show_actions=False, task_id=None):
    st.markdown(f'<div style="background:#1a1a1a; color:#ffcc00; padding:15px; font-weight:900;">KALUSTOKORTTI | {k.get("nimi","")}</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="background:#ffcc00; color:black; padding:15px; font-size:1.8rem; font-weight:900; margin-bottom:20px;">{k.get("rekisteri", "EI REK")}</div>', unsafe_allow_html=True)
